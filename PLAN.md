...
- [ ] 初始化 Tauri v2 + Svelte 5 项目
- [ ] 设计 SQLite 数据库 schema（加密条目表、元数据表）
- [ ] 实现 Argon2id 密钥派生模块 (Rust)
- [ ] 实现 AES-256-GCM 加密/解密模块 (Rust)
- [ ] 实现主密码设置与验证流程
- [ ] 实现保险库锁定/解锁 (内存中持有密钥的状态管理)
- [ ] 密码条目基本 UI：列表展示 + 新增/编辑/删除
- [ ] 集成测试：加密 → 存储 → 解密 → 验证 完整流程
...
- [ ] 密码生成器：可配置长度、大小写、数字、特殊字符、排除字符
- [ ] 密码强度实时评估（zxcvbn 算法或简化版）
- [ ] 搜索功能：按标题、域名、用户名模糊搜索
- [ ] 文件夹/标签分组系统
- [ ] 收藏/置顶功能
- [ ] 密码条目 UI 完善：显示/隐藏密码、一键复制
