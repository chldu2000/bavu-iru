...
<div class="w-72 rounded-lg border border-dark-border bg-dark-card p-3 shadow-xl">
  <div class="mb-3 flex items-center justify-between">
    <span class="text-xs font-medium text-dark-secondary">密码生成器</span>
  </div>

  <!-- Generated password preview -->
  <div class="mb-3 rounded-md border border-dark-border bg-dark-bg px-2 py-1.5 font-mono text-xs break-all text-dark-text">
    {generated || '...'}
  </div>
  ...
  <div class="mb-3">
    <div class="mb-1 flex items-center justify-between">
      ...
      <span class="text-xs text-dark-text">{options.length}</span>
    </div>
    <input
      type="range"
      min="8"
  ...
  <div class="mb-3 grid grid-cols-2 gap-2">
    <label class="flex cursor-pointer items-center gap-1.5 text-xs text-dark-secondary">
  ...
  <!-- Exclude chars -->
  <div class="mb-3">
    <span class="mb-1 block text-xs text-dark-muted">排除字符</span>
    <input
      type="text"
      bind:value={options.exclude_chars}
      placeholder="例如: 0OlI1"
      class="w-full rounded-md border border-dark-border bg-dark-bg px-2 py-1 text-xs text-dark-text outline-none placeholder:text-dark-muted focus:border-accent"
    />
  </div>
  ...
  <div class="flex gap-2">
    <button
      class="flex-1 cursor-pointer rounded-md border border-dark-border bg-dark-bg px-3 py-1.5 text-xs text-dark-secondary hover:text-dark-text"
      onclick={regenerate}
    >
      重新生成
    </button>
